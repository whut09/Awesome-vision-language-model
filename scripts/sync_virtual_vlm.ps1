<# Collect ICLR/ECCV VLM papers from their official virtual proceedings. #>

param(
    [ValidateSet('ICLR', 'ECCV')]
    [string]$Conference,
    [int]$Year
)
...
$anchorPattern = '(?is)<a href="(?<href>/virtual/' + $Year + '/(?:poster|oral)/\d+)">(?<title>.*?)</a>'
$papers = [regex]::Matches($index.Content, $anchorPattern) |
